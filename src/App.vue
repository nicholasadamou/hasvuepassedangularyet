<template>
  <div id="app">
    <github-corner/>
    <p>Has Vue passed Angular yet?</p>
    <template v-if="repos">
      <h1 v-if="!tie">{{ vueHasPassedAngular ? 'YES' : 'NO' }}</h1>
      <h1 :class="{ pad : tie }" v-else>TIE!</h1>
      <p>
        <small v-if="!vueHasPassedAngular && !tie" class="away">
          Only {{ angularStars - vueStars | formatNumber }} {{ angularStars - vueStars === 1 ? 'star' : 'stars'}} away!
        </small>
        <small v-else-if="vueHasPassedAngular && !tie" class="ahead">
          Ahead by {{ vueStars - angularStars | formatNumber }} {{ vueStars - angularStars === 1 ? 'star' : 'stars'}}!
        </small>
      </p>
      <ul>
        <li>
        ...
        <li>
          <a :href="repos.angular.url" target="_blank">
            <angular-icon/>
...
import { VueIcon, AngularIcon, StarIcon } from "./components/icons";

const FUNCTIONS_ENDPOINT =
  "https://wt-55b2b1c7748a70b57be70645c525e42a-0.sandbox.auth0-extend.com/fetchGithubStars";
...
export default {
  ...
  computed: {
    vueHasPassedAngular() {
      return this.vueStars > this.angularStars;
    },
